            ...
            obrasList.innerHTML = ''
             ...
             obras.forEach(obra => {
                ...
                obraElement.innerHTML = `
                    <h3 class="text-x1 font-semibold text-blue-500">${obra.name}</h3>
                    <p class="text-gray-600">${obra.description || 'Sem descrição'}</p>
                    <p class="text-sm text-gray-500">Status: ${obra.status || 'Não definido'}</p>
                    <p class="text-sm text-gray-500">Início: ${obra.startDate ? new Date(obra.startDate).toLocaleDateString() : 'Não definido'}</p>
                    <div class="mt-2">
                        <button onclick="deleteObra(${obra.id})" class="text-red-500 hover:text-red-700 text-sm mr-2>Excluir</button>
